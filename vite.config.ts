...
const githubPinnedMiddleware = () => ({
  ...
  configureServer(server: any) {
    server.middlewares.use('/api/github-pinned', async (req: any, res: any, next: any) => {
      if (req.method !== 'GET') {
        res.statusCode = 405;
      ...
      try {
        ...
        const repos = (data?.data?.user?.pinnedItems?.nodes ?? []).map((repo: any) => ({
          ...
          topics: (repo.repositoryTopics?.nodes ?? []).map((topicItem: any) => topicItem.topic?.name).filter(Boolean),
        }));
